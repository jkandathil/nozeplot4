...
export const GUIDE_SECTIONS = [
    ...
    },
    {
        ...
        intro:
            '**MCU Flash** is a browser-based programmer for Arduino-family AVR boards (Uno, Nano, Pro Mini) and the ESP32 / ESP8266 family. Edit Arduino C/C++ in a Monaco editor, get a complete sketch from the built-in **MCU agent**, then click **Build & Flash** — the app compiles your editor source in the cloud and programs the board over **Web Serial**.',
        fundamentals: [
            '**Build & Flash:** compiles the current editor source (main `.ino` + extra `.cpp`/`.h` + libraries) via a cloud build service, converts to AVR `.hex` or ESP `.bin`, then flashes in-browser.',
            '**Cloud build service:** a hosted arduino-cli wrapper (`tools/arduino-compile-server/`) deployed with the app (Cloud Run or Render). Configured via `VITE_MCU_COMPILE_URL` at build time — no Arduino IDE or manual `.hex` export.',
            '**Projects, not just files:** the side pane is a **Project Explorer** — each project is a multi-file app (a main `.ino` plus any `.cpp`/`.h` files) with its own external-library list. The **★ main** file is the compile entry point. Everything persists in your browser.',
            '**Import & save:** **Import** (↓) pulls `.ino`/`.cpp`/`.h` files from disk into the active project; **Import from workspace** loads sketches previously saved to the `Arduino` workspace folder; **Save** writes the whole project back to the workspace.',
            '**External libraries:** open **Libraries** to add dependencies by their Arduino Library Manager name (optionally `name@version`). They are installed by the compile bridge before each build.',
            '**Flashing in the browser:** ESP32/ESP8266 use **esptool-js**; AVR boards use **STK500 v1**. Both run over Web Serial in Chromium (Chrome / Edge) over HTTPS or localhost.',
            '**Compile bridge contract:** `POST /compile` with `{ fqbn, sketch, sketchName, files, libraries }`, returning base64 `hex` (AVR) or `parts` (ESP). Bundled in `tools/arduino-compile-server/`.',
            '**Large-scale AI development:** the MCU agent sees the whole project (every file + libraries) and replies with one fenced block per file (the filename is the fence label, e.g. ```cpp sensors.h```). Multi-file replies are applied across the project, creating or updating files as needed.',
        ...
        implemented: [
            'Project Explorer side pane: multiple projects, multiple files per project, set **main** sketch, create/rename/delete',
            'Import code from disk (`.ino`/`.cpp`/`.h`, multi-select) and from the `Arduino` workspace folder',
            'Per-project **external library** manager forwarded to the compile bridge',
            'Monaco C/C++ editor; **Save** the whole project to the workspace; **Save firmware** (.hex/.bin)',
            'Board selector: Arduino Uno / Nano / Pro Mini / Duemilanove · ESP32 / S2 / S3 / C3 / C6 · ESP8266',
            'ESP flashing via esptool-js; AVR flashing via STK500 v1',
            '**Build & Flash** — compile editor source in the cloud, then program the board in one click',
            'Cloud build service status indicator (build ready / build offline)',
            'Built-in serial monitor (send box, line endings, live log) with auto-reconnect after flashing',
            'Multi-file **MCU agent**: whole-project context, multi-file apply, and a **Build ▶ Flash ▶ Observe** pipeline',
        ],
        steps: [
            'Create a **project** (folder icon) or use the default one. **Import** existing code, or add files with the **+** on a project.',
            'Pick your **board**; if your app needs libraries, open **Libraries** and add them by name.',
            'Write code, or ask the **MCU agent** (e.g. “read DHT22 and serve JSON over Wi-Fi, split into files”). It can create several files at once.',
            'Click **Select port** and choose the Uno/ESP device. Confirm the toolbar shows **build ready**.',
            'Click **Build & Flash** (watch the **Console**), then **Serial Monitor** for output. The MCU agent can also run **Build ▶ Flash ▶ Observe** autonomously.',
        ],
    },
    {
        id: 'code-studio',
